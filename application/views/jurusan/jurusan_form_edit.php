			...
			<div class="col-md-6">
				<form action="" method="POST">
					...
					<div class="form-group <?= form_error('idJurusan') ? 'text-danger' : null ?>">
						...
						<input type="text" name="idJurusan" id="idJurusanInput" value="<?= $this->input->post('idJurusan') ?? $row->id_jurusan ?>" class="form-control <?= form_error('idJurusan') ? 'is-invalid' : null ?>" placeholder="ID Jurusan">
					...
					<div class="form-group <?= form_error('namaJurusan') ? 'text-danger' : null ?>">
						...
						<input type="text" name="namaJurusan" id="namaJurusanInput" value="<?= $this->input->post('namaJurusan')  ?? $row->nama_jurusan ?>" class="form-control <?= form_error('namaJurusan') ? 'is-invalid' : null ?>" placeholder="Nama Jurusan">
